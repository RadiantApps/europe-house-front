        ...
        <nav className="hidden lg:flex gap-8 items-center">
          {navLinks[selectedLanguage]?.map((link, index) => (
            <Link
              key={index}
              href={link.href}
              className="text-[16px] text-black hover:text-blue-600 kanit-regular"
            >
              {link.label}
            </Link>
          ))}
      ...
      >
        <div className="flex flex-col p-6 space-y-6 ">
          ...
          <div className="border-b border-gray-200 pb-4 overflow-auto">
            <label className="block text-[16px] font-medium text-gray-700 mb-2 kanit-regular">
              Language
            </label>
            <select
              className="w-full border border-gray-300 rounded-md text-[16px] px-3 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 kanit-regular"
              value={selectedLanguage}
              onChange={handleLanguageChange}
            >
              <option value="en">English</option>
          ...
          {/* Navigation Links */}
          <nav className="flex flex-col space-y-4 kanit-regular">
            {navLinks[selectedLanguage]?.map(({ href, label }) => (
              <Link
                key={href}
                href={href}
